...
export const register = createAsyncThunk("auth/register", async({ name, lastname, email, password, confirmPassword }, {rejectWithValue}) => {
    ...
    try {
        ...
        const response = await axios.post("https://www.elvestidordejuliettaapi.tk/api/v1/user/register", {
            name,
            lastname,
            email,
            password,
...
export const login = createAsyncThunk("auth/login", async( { email, password }, {rejectWithValue} ) => {
    ...
    try {
        const response = await axios.post("https://www.elvestidordejuliettaapi.tk/api/v1/user/login", {
            email,
            password
            });
